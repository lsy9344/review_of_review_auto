...
class ResultsWidget(QWidget):
    ...
    def create_results_table_tab(self):
        ...
        self.results_table.setHorizontalHeaderLabels([
            "시간", "매장명", "리뷰내용", "생성된 답글", "상태", "오류", "소요시간", "재시도"
        ])
        
        # 테이블 설정
        header = self.results_table.horizontalHeader()
        header.setSectionResizeMode(0, QHeaderView.ResizeMode.ResizeToContents)  # 시간
        header.setSectionResizeMode(1, QHeaderView.ResizeMode.ResizeToContents)  # 매장명
        ...
        header.setSectionResizeMode(6, QHeaderView.ResizeMode.ResizeToContents)  # 소요시간
        header.setSectionResizeMode(7, QHeaderView.ResizeMode.ResizeToContents)  # 재시도
    ...
    def create_error_tab(self):
        ...
        error_stats_frame.setStyleSheet("background-color: #fff3cd; border: 1px solid #ffeaa7; border-radius: 4px;")
        
        error_stats_layout = QHBoxLayout(error_stats_frame)
        
        self.error_stats_label = QLabel("오류 통계: 로그인 오류 0건, DOM 오류 0건, LLM 오류 0건, 네트워크 오류 0건")
    ...
    def add_result(self, result_data: dict):
        ...
        review_text = result_data.get("review_text", "")[:100] + "..." if len(result_data.get("review_text", "")) > 100 else result_data.get("review_text", "")
        reply_text = result_data.get("reply_text", "")[:100] + "..." if len(result_data.get("reply_text", "")) > 100 else result_data.get("reply_text", "")
    ...
    def add_error(self, error_type: str, error_message: str, details: str = "", solution: str = ""):
        """오류 추가"""
    ...
    def update_statistics(self):
        ...
        self.last_run_label.setText(f"마지막 실행: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    def export_data(self, format_type: str):
        ...
        file_filter = "CSV 파일 (*.csv)" if format_type == "csv" else "JSON 파일 (*.json)"
        file_path, _ = QFileDialog.getSaveFileName(self, "결과 내보내기", "", file_filter)
        
        if file_path:
            self.export_requested.emit(format_type)
            # 실제 내보내기는 향후 구현
            QMessageBox.information(self, "알림", f"{format_type.upper()} 파일로 내보내기 완료")
    
    def clear_results(self):
        """결과 지우기"""
        reply = QMessageBox.question(self, "확인", "모든 결과를 지우시겠습니까?",
    ...
    def save_log(self):
        """로그 저장"""
        file_path, _ = QFileDialog.getSaveFileName(self, "로그 저장", "", "텍스트 파일 (*.txt)")
        if file_path:
            try:
                ...
                QMessageBox.critical(self, "오류", f"로그 저장 중 오류가 발생했습니다: {str(e)}")
